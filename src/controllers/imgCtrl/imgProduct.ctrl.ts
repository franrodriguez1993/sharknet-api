...
import {
  createImgProductServ,
  delImgProductServ,
} from "../../services/imageServ/imgProduct.serv";

/** ==================== CREATE IMG PRODUCT ===================== **/

export async function createImgProductCtrl(req: RequestExt, res: Response) {
  try {
    //Data:
    const { file } = req;
    ...
    //Service:
    const img = await createImgProductServ(req.uid, data);
...
/** ==================== DELETE IMG PRODUCT ===================== **/
export async function delImgProductCtrl(req: Request, res: Response) {
  try {
    //Data:
    const { id } = req.params;
    //Service:
    const del = await delImgProductServ(id);
